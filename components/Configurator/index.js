...
export default class Configurator extends React.Component {
    ...
    */
        ...
        this.setState({persistentData: data, dataLoaded: true, configVisible: false})
        ...
        AccountStorageQuery.query({
            accountId: this.props.accountId,
            collection: this.props.storageCollectionId,
            fetchPolicyType: AccountStorageQuery.FETCH_POLICY_TYPE.NO_CACHE
        }).then(({ data }) => {
            data.forEach((doc)=>{
        ...
        AccountStorageQuery.query({
            ...
            if(metaData) {
                ...
                if (hasDefault) {
                    ...
                    //this.saveToStorage(this.props.default); //this can be buggy, instead lets allow user to choose to save.
                }
                else {
    ...
    render() {
        ...
        let returnVal, configPanel
        if(configVisible) {
            configPanel=<div><Spinner inline />Loading configuration...</div>
            if(dataLoaded) {
                ...
                configPanel=<div className="configContainer" hidden={!configVisible} onClose={()=>{this.setState({configVisible: false})}} >
